// Export du synopsis en PDF (P38, rôle Rédacteur). L'arabe est rendu via une
// mise en page HTML capturée par html2canvas (le navigateur façonne l'arabe et
// gère le RTL — jsPDF en mode texte ne sait pas le faire), puis l'image est
// paginée sur des pages A4. Rien n'est stocké : téléchargement direct.
...
const SUFFIXE_FICHIER = { FR: 'FR', AR: 'AR', BILINGUE: 'FR-AR' }
...
function construireNoeud({ programme, chaineNom, synopsisFr, synopsisAr, langue }) {
  ...
  if (langue !== 'AR') {
    blocs.push(
      ...
      `<div style="white-space:pre-wrap;font-size:14px;margin-bottom:28px">${echapper(synopsisFr) || '<span style="color:#94a3b8">(vide)</span>'}</div>`
    )
  }
  if (langue !== 'FR') {
    blocs.push(
